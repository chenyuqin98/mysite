...
class EconomySpider(scrapy.Spider):
    ...
    start_urls = ['https://libgen.lc/search.php?req=Economy&open=0&res=100&view=simple&phrase=1&column=def']

    def parse(self, response):
        for i in range(1, 10):
            yield scrapy.Request(
                url='https://libgen.lc/search.php?&res=100&req=Economy&phrase=1&view=simple&column=def&sort=def&sortmode=ASC&page=' + str(
                    i)
